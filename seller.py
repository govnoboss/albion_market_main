...
def ocr_scan_text(region):
    ...
    try:
        ...
        custom_config = (
    r'-c tessedit_char_blacklist=abcdefghijklmnopqrstuvwxyz'
    r'ABCDEFGHIJKLMNOPQRSTUVWXYZ'
    r'0123456789'
    r'!@#$%^&*_=+[]{};:\'",.<>?/\\|`~'
    r' --psm 6'
)
        text = pytesseract.image_to_string(screenshot, config=custom_config, lang='rus').strip()
        return text.lower()
...
def run_selling_cycle(coords, regions, total_spent, gui_callback):
    """
    Основной цикл продажи предметов с расширенной отладкой.
    """
    log_entries = []
    ...
    # --- Шаг 2: Загрузка таблицы table.xlsx ---
    try:
    ...
    while failure_counter < 5:
        ...
        try:
            if not row_purchase.empty:
                ...
                wapp = item_data['wapp']
                log_and_update(f"[DEBUG] WAPP для {item_name}: {wapp}")
        ...
        if sale_price > 0:
            try:
                ...
                quantity_in_stack = int(sale_price / ((price_per_item-1) * (1 - tax))) if price_per_item > 1 else 0
                ...
                log_and_update(f"[DEBUG] Профит за 1 предмет: {profit_per_item}")
                ...
                log_and_update(f"[DEBUG] Продажа подтверждена: доход={total_income}, прибыль за шт={profit_per_item}")
            except Exception as e:
                log_and_update(f"[ERROR] Ошибка при подтверждении продажи: {e}")
        else:
            log_and_update(f"[DEBUG] Цена продажи не распознана, пропуск предмета {item_name}")
            time.sleep(0.2)
    ...
    report_lines = [f"--- Итог сессии ---", f"Доход: {total_income}", f"Расход: {total_spent}", f"Прибыль: {final_profit}"]
    for item in sold_items_details:
        report_lines.append(f"{item['name']} x{item['quantity']} | Продажа: {item['sale_price_total']} | Прибыль за шт: {item['profit_per_item']}")
